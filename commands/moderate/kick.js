...
module.exports = {
    ...
    async run (client, message, args){
        ...
        if(!message.member.permissions.has("KICK_MEMBERS")) return message.channel.send({ embeds: [errr] })
        ...
        await mentionedMember.kick({
            reason: reason
        }).catch(err => console.log(err)).then(() => message.channel.send({ embeds: [kickEmbed] }))
